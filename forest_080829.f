...
c     locals
      ...
      double precision Ibess(nmax),Ibessp(nmax),Ibesspp(nmax)
      double complex   dp
      double complex A(nmax),B(nmax)
      double complex i
...
c-----lambda = 0.5 k_perp^2 * rho_e^2
      ...
      write(*,*)'dhot_s nps,beta,tpop,Y,lambda',nps,beta,tpop,Y,lambda
...
c      write(*,*)'in DHOT_s before harmon_z nmax',nmax     
     ...
     .n_harm1,n_harm2,n,istop)
      ...
      write(*,*)'in DHOT_s after harmon_z nmax',nmax,'istop',istop
      write(*,*)'dhot_s n',n
      write(*,*)'in DHOT_s after harmon_z mass,n_harm1,n_harm2,n,istop',
     .mass,n_harm1,n_harm2,n,istop
...
c     .    n_harm1,n_harm2,n,istop)
     ...
     .     n_harm1,n_harm2,n_new,istop)
          ...
          write(*,*)'dhot_s after harmon_z n,n_new',n,n_new
c          goto 10
           goto 9          
        endif
      enddo
...
cSm060816
 9    continue
      n=n_new
      write(*,*)'in DHOT_s 2 n,n_harm1,n_harm2',n,n_harm1,n_harm2
...
c      parameter (nmax=3)      
     ...
     .vflow,vflowp,betat_av,betatpop
      double precision dr,di,xne(nmax),drp,Ibpp(nmax)
      double precision Ibess(nmax),Ibessp(nmax),Ibesspp(nmax),Ibp(nmax)

      double complex A(nmax),B(nmax),Ap(nmax),Bp(nmax)
...
c      write(*,*)'dk_tens_s aft harmon_z nmax',nmax      

      if (istop.eq.0) then
c         write(*,*)'DKtens_s istop=0 it is impossible to calulate' 
c         write(*,*)'the harmonics for susceptibily tensor n_harm0,nmax'
c     .   ,nmax
      endif
...
c        do j1=1,3
c          do j2=1,3
c            K_sum(j1,j2)=K_sum(j1,j2)+K(j1,j2)
c            write(*,*)'j1,j2,K_sum(j1,j2)',j1,j2,K_sum(j1,j2)
c          enddo
c         enddo
...
c        write(*,*)'hotdervs nbulk,rholoc',nbulk,rholoc
...
c	  write(*,*)'i,x_ar(i),y_ar(i)',i,x_ar(i),y_ar(i)
c          write(*,*)'t_keV,t_av_ar(i),tpop_ar(i),vflow_ar(i)',
c     &               t_keV,t_av_ar(i),tpop_ar(i),vflow_ar(i)
	enddo
...
c           write(*,*)'alpha',alpha
      ...
      norm_bot=norm_top
      write(*,*)'forest.f i,deltn,norm_top',i,deltn,norm_top
...
cSm020410
      double complex Ibess(nmax),Ibessp(nmax),Ibesspp(nmax)
      double precision br(nmax),bi(nmax) !real and imaginary modified bessel func. I_n      
