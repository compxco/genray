...
# specify:
level_mn= 165 #23 # Lowest level of omega/omega_c to be plotted
level_mx= 167 #45 # Highest level of omega/omega_c to be plotted
...
if ncontr > 0:  # plot LCFS, if any
    plt.plot(multiply(rcontr,100),multiply(zcontr,100),'g',linewidth=linw*4)
...
for i in range(0,Nrays,1):  # i goes from 0 to Nrays-1
    ...
    if Nm>0:
        ...
        dwidth=50.0*width_diff/np.max(width_diff) 
...
plt.subplot(223) #------------------------- J||
txt="$I_{CD}$"+"$=$%3.3f" %(parallel_cur_total/1e3) +" $MA$"
title(txt,x=1.5)
plt.hold(True)
plt.grid(True)
...
for i in range(0,Nrays,1):  # i goes from 0 to Nrays-1
    ...
    Nm= nrayelt[i] # max number of points along a ray
    if Nm>0:
        semilogy(ws[i,0:Nm],delpwr[i,0:Nm]/1e13,linewidth=linw_mx-i*dlinw)
