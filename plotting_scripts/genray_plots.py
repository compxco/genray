...
# specify:
level_mn=1 #15 # 1 #23 # Lowest level of omega/omega_c to be plotted
level_mx=3 #25 # 40 #45 # Highest level of omega/omega_c to be plotted
...
rays_style=0 #2 # 0 Rays in (R,Z) are plotted with constant line thickness.
             ...
             #    absorbed power.
isp_wc=1 #2 #4 #2 #2  #  Species number for which you want to plot omega/omega_c res.layers
...
Rmin_plot=0. #50. #150. #95. #70.
Rmax_plot=0. #170. #600. #750. #650. #430. 
Zmin_plot=-50. #-400.
Zmax_plot=+50. # 400.

# Additional structures (ports, coils) to be plotted in (R,Z) section
istruct=0 #1 #0 # To skip, set istruct=0
...
#eqdsk_name='eqdsk'
#eqdsk_name='g130608.00352.LRDFIT04.mds.corrected.qscale_1.00000'

#eqdsk_name='jetto_eqdsk_SPR14' # [2022-04] -------------GOOD
#btor [Tesla]= 4.0, toteqd [A]= 20978463.5 GOOD

#eqdsk_name='jetto_eqdsk_SPR08' # [2022-04] STEP/HHFW   BAD
#eqdsk_name='jetto_eqdsk_SPR08_posBtor_posItor' #--------GOOD
#btor [Tesla]= 2.29999995, toteqd [A]= 20980050.0 
#eqdsk_name='jetto.eqdsk_SPR46_posBtor_posItor'

#eqdsk_name='SCENE_SPR_16MA_flattop_strawhighq0_v1-0-0.geqdsk'

#eqdsk_name='eqdsk_MAST-like' # for test10
#eqdsk_name='eqdsk'

#eqdsk_name='FENIX_PT_XPT__08122024'
#eqdsk_name='93090A21_890ms.eqdsk' #SMART/ N.Bertelli [2025-12]
...
#filenm='genray_LH.1.nc'
#filenm='STEP_SPR08_helicon400.nc'
#filenm='STEP_SPR14_helicon400.nc'

#filenm='STEP_SPR46_helicon200.nc' #2022-08-15
#filenm='STEP_SPR46_helicon300.nc' #2022-07-17, 08-15
#filenm='STEP_SPR46_helicon400.nc' #2022-08-15
#filenm='STEP_SPR46_helicon500.nc' #2022-07-17, 08-15
#filenm='STEP_SPR46_helicon600.nc' #2022-08-15
...
try:# Try reading eqdsk file, hoping it exists in the working directory.
    ...
    print 'psimag,psilim,min(epsi),max(epsi)',psimag,psilim,np.min(epsi),np.max(epsi)
    #stop
    #........................................................
        ...
        for iz in range(0,nz,1):  # iz goes from 0 to nz-1
            ...
            if psilim>psimag:
                ...
                if epsi[iz,ir]>psilim:
                    ...
                    #if iz==64: print epsi[iz,ir], iv, psiar[iv],psiar[iv+1]
                    #if iv<=0:
                    #    print epsi[iz,ir],psimag,delpsi, ir,iz,iv
                    Btor[iz,ir]=0 #fpsiar[iv]/er[ir]
                    Bpmn=min(Bpol[iz,ir],Bpmn)
                    Bpmx=max(Bpol[iz,ir],Bpmx)
                else:
                    Btor[iz,ir]=btor*radmaj/er[ir]            
...
nbulk=dat.variables['nbulk']
nbulk=nbulk.getValue()
...
for isp in range(0,Nsp,1):
    ...
    print 'isp=',isp,' mass=',mass[isp],' Z=',charge[isp], mass[isp]/mp
    ...
    if (np.absolute(Zs-4.)<0.1) & (np.absolute(msme-9*mp)<200):
        isp_name.append('Be+4')
    if (np.absolute(Zs-4.)<0.1) & (np.absolute(msme-12*mp)<200):
        isp_name.append('C+4')
    if (np.absolute(Zs-5.)<0.1) & (np.absolute(msme-12*mp)<200):
        isp_name.append('C+5')
    if (np.absolute(Zs-6.)<0.1) & (np.absolute(msme-12*mp)<200):
        isp_name.append('C+6')
    if (np.absolute(Zs-10.)<0.1) & (np.absolute(msme-20*mp)<800):
        isp_name.append('Ne+10')
    if (np.absolute(Zs-18.)<0.1) & (np.absolute(msme-40*mp)<800):
        isp_name.append('Ar+6')
    if (np.absolute(Zs-54.)<0.1) & (np.absolute(msme-131*mp)<2000):
...
iabsorp=dat.variables['iabsorp']
iabsorp= iabsorp.getValue()
print( 'iabsorp=',iabsorp)
...
# Temperature along rays [keV]
ste=dat.variables['ste']
print('ste is: ', ste.long_name, ste.shape, ste.units) #[nrays;neltmax]
# To be used in
...
if iabsorp==3:
    powtot_s= dat.variables['powtot_s']
    powtot_s= np.asarray(powtot_s)/1.e10
...
plt.grid(True)
plt.ylim((0.,1.05*np.max(temprof)))
...
#print 'densprof[Nsp]/1e13  cm-3='
#for ir in range(0,150):
#    print ir,densprof[Nsp-1,ir]/1e13
#stop
...
plt.xlim((-Rmax_plot, Rmax_plot))
plt.ylim((-Rmax_plot, Rmax_plot))
...
for i in range(0,Nrays,1):  # i goes from 0 to Nrays-1
    ...
    if Nm>0:
        ...
        if rays_style==0: # Usual style - solid line
        ...
        if rays_style==1:
            ...
            plot_widths(X,Y,width,color=col)
        if rays_style==2:           
            plt.plot(X,Y,color=col,linewidth=linw*0.5) #Usual
            #--->Rays with width corr. to absorbed power:
            width=(delpwr[i,0:Nm]/delpwr[i,0])
            width_diff= np.diff(width) #diff(width) is neg when power is reduced
            dwidth=100.0*np.abs(width_diff)/np.max(np.abs(width_diff)) 
            for irayel in range(0,Nm-1,1):
                if width[irayel]<1.0: # Power is reduced -- wave damping
                    plt.scatter(X[irayel],Y[irayel],\
                    marker='o',edgecolor="red",facecolor="none",s=dwidth[irayel]) # as markers
                    #print width[irayel]
                else: # width[irayel]>1.0: #Power is increased -- wave growth
                    plt.scatter(X[irayel],Y[irayel],\
                    marker='o',edgecolor="blue",facecolor="none",s=dwidth[irayel]) # as markers
...
if i_eqdsk==1: # Plot resonance layers for electrons or ions
    CS=plt.contour(R,Z,WWc,levels=levels,linewidths=1,cmap=plt.cm.get_cmap("jet"))
    plt.clabel(CS,inline=True,inline_spacing=1.,\
    fmt='%1.0f',fontsize=fnt, colors='k',rightside_up=False)
    #plt.clabel(CS,inline=True,manual=True,\
    #fmt='%1.0f',fontsize=fnt, colors='k',rightside_up=False)
...
if n_wall+ncontr+nlimiter >4: # no data on walls or LCFS; Try plotting PSI
    Ncont=10
    if n_eqdsk==1:
        R,Z = np.meshgrid(eqdsk_r, eqdsk_z)
    ...
    if n_eqdsk==0: # There was no data on PSI. Use data from eqdsk
        Req,Zeq = np.meshgrid(er,ez) # 2D grids [cm]
        PSI = epsi
        #CS=plt.contour( Req,Zeq,PSI,Ncont,linewidths=1,cmap=plt.cm.get_cmap("Dark2"))
        #CB=plt.colorbar(orientation='vertical', shrink=0.5, format='%.2e')
        # Levels of PSI (to mark psimag and psilim)
...
for i in range(0,Nrays,1):  # i goes from 0 to Nrays-1
    ...
    if Nm>0:
        ...
        if rays_style==2:           
            plt.plot(wr[i,0:Nm],wz[i,0:Nm],color=col,linewidth=linw*0.5) #Usual
            #--->Rays with width corr. to absorbed power:
            width=(delpwr[i,0:Nm]/delpwr[i,0])
            width_diff= np.diff(width) #diff(width) is neg when power is reduced
            dwidth=100.0*np.abs(width_diff)/np.max(np.abs(width_diff)) 
            print 'i,Nm=',i,Nm, '  shape(dwidth)',shape(dwidth)
            for irayel in range(0,Nm-1,1):
                if width[irayel]<1.0: # Power is reduced -- wave damping
                    plt.scatter(wr[i,irayel],wz[i,irayel],\
                    marker='o',edgecolor="red",facecolor="none",s=dwidth[irayel]) # as markers
                    #print width[irayel]
                else: # width[irayel]>1.0: #Power is increased -- wave growth
                    plt.scatter(wr[i,irayel],wz[i,irayel],\
                    marker='o',edgecolor="blue",facecolor="none",s=dwidth[irayel]) # as markers
                    #print width[irayel]
                #if abs(spsi[i,irayel]-0.5)<0.005 :   #Markers where rho~0.5
                #    plt.scatter(wr[i,irayel],wz[i,irayel],\
                #    marker='s',edgecolor="k",facecolor="none",s=dwidth[irayel]) # as markers
                #if abs(spsi[i,irayel]-0.7)<0.005 :   #Markers where rho~0.7
                #    plt.scatter(wr[i,irayel],wz[i,irayel],\
                #    marker='s',edgecolor="k",facecolor="none",s=1.5*dwidth[irayel]) # as markers
...
if (sb_z[0,0]>0)&(wr[0,0]>R_axis):   # sb_z is Bz along each ray
...
print( 'q2m=charge[0]**2/mass[0] = ', q2m )
...
for i in range(0,Nrays,1):  # i goes from 0 to Nrays-1
    ...
    Nm= np.asscalar(nrayelt[i])-nskip # max number of points along a ray
    if Nm>0:
        w_wce= f/abs(28e5*qm*sbtot[i,0:Nm]) # qm is always for electrons here
        plt.plot(ws[i,0:Nm],w_wce,color=col,linewidth=linw)
plt.xlabel('$pol.dist$ $along$ $ray(cm)$')

plt.subplot(232) #------------------------- value of RF frequency ,
                 # and N||acc = sqrt(1 + wpe^2/wce^2) 
plt.hold(True)
plt.grid(True)
plt.xlim( (ws_min, ws_max) )
txt="$ n_{||acc}=(1+(\omega_{pe}/\omega_{ce})^2)^{0.5}$"
...
for i in range(0,Nrays,1):  # i goes from 0 to Nrays-1
    ...
    Nm= np.asscalar(nrayelt[i])-nskip # max number of points along a ray
    if Nm>0:
        w_wce_2= (f/(28e5*qm*sbtot[i,0:Nm]))**2  # (w/wce)^2 
        # qm,q2m always for electrons here
        wpe_w_2= 806.2e5*sene[i,0:Nm]*q2m/f**2   # (wpe/w)^2
        Npar_acc= sqrt(1.0 + wpe_w_2*w_wce_2) 
        # accessibility for LH is Npar>Npar_acc 
        plt.plot(ws[i,0:Nm],Npar_acc,color=col,linewidth=linw)
...
if Nsp>1: # plot w_c/w along rays
    ...
    plt.title('$|\omega_{c}/\omega|$ $[$'+isp_name[isp]+'$]$')
    #xticks(np.arange(ws_min, ws_max, step=ws_step) )
    plt.xlim( (ws_min, ws_max) )
...
for i in range(0,Nrays,1):  # i goes from 0 to Nrays-1
    ...
    if Nm>0:
        ...
        NII=  wnpar[i,0:Nm]
        Vte=  1.87e9*sqrt(ste[i,0:Nm]/mass[0]) # Vte [cm/s]
        # vth=sqrt(2T/m),    vth in (cm/sec),T(keV)
        # vth= 1.87e9*sqrt(T/dmas)
        VV= (1-nres*wc_w)/(NII*Vte/clight)
        #The above is V||res/Vt  = (omega-nres*omega_c)/(K||*Vt)
...
if iabsorp==3:
    powden_s=np.asarray(powden_s)*pwrscale
...
if iabsorp==3:
    powden_s=np.asarray(powden_s[:,:])/1.0e4 # kW/m^3
...
if iabsorp==3:
    for i_s in range(0,nbulk-1,1):
        # Note that in GENRAY powtot_s is set over kk=2,nbulk,
        # saved from index 2 [powtot_s(2:)],
        # but in Python the numeration starts from 0
        #print i_s, shape(powden_s), shape(rho_bin_center) 
        #plt.plot(rho_bin_center,(powden_s[i_s,:]),'k--',linewidth=linw)
        print(isp_name[i_s+1],' ion species k=',i_s+1,' P_k [kW]=', powtot_s[i_s] )
axis([0.,rhomax,0.,1.05*np.amax(powden_i)+1.e-3])
#axis([0.,1., 0., 0.35])
...
linw_mx= 2*linw # largest thickness
linw_mn= 0.5    # smallest
if Nrays>1:
...
for i in range(0,Nrays,1):  # i goes from 0 to Nrays-1
    ...
    if Nm>0:
        ...
        plt.plot(ws[i,0:Nm],delpwr[i,0:Nm]/1e13,'k.')

plt.subplot(212) #-------------------------
# plt.hold(True)
plt.grid(True)
plt.title('$n_{||}$ $along$ $ray$',y=0.8)
xticks(np.arange(ws_min, ws_max, step=ws_step) )
xticks(np.arange(ws_min, ws_max, step=ws_step) )
plt.minorticks_on() # To add minor ticks
plt.tick_params(which='both',  width=1)
...
plt.tick_params(which='minor', length=4, color='k') 
plt.xlim( (ws_min, ws_max) )
y_mn= np.min(wnpar) # second arg: Nm=points_along_ray (can be 0)
y_mx= np.max(wnpar)
dy= 0.35*(y_mx-y_mn)
...
for i in range(0,Nrays,1):  # i goes from 0 to Nrays-1
    ...
    if Nm>0:
        plt.plot(ws[i,0:Nm],wnpar[i,0:Nm],color=col,linewidth=linw_mx-i*dlinw)
...
plt.tick_params(which='major', length=7)
plt.tick_params(which='minor', length=4, color='k') 
plt.xlim( (rho_min, max(rho_max, 1.2)) )
#plt.ylim( (delpwr_min, delpwr_max*2) )
plt.ylim( (-0.05, 1.05) )
# Plot with different thickness of lines, depending on ray
linw_mx= 2*linw # largest thickness
linw_mn= 0.5    # smallest
...
for i in range(0,Nrays,1):  # i goes from 0 to Nrays-1
    ...
    Nm= nrayelt[i] # max number of points along a ray
    if Nm>0:
        plt.plot(spsi[i,0:Nm],delpwr[i,0:Nm]/1e13,linewidth=linw_mx-i*dlinw)
        plt.plot(spsi[i,0:Nm],delpwr[i,0:Nm]/1e13,'k.')
        plt.plot(spsi[i,0],delpwr[i,0]/1e13,'ko') # mark starting point
        spsi_1=spsi[i,0:Nm]
        spsi_diff= np.abs(np.diff(spsi_1))
        print('d(rho) along ray: max=', np.max(spsi_diff))
        # Note: the max value of d(rho) should be smaller than d(rya) in CQL3D
        #print('rho (=spsi) min/max=', np.min(spsi_1),np.max(spsi_1))
...
for i in range(0,Nrays,1):  # i goes from 0 to Nrays-1
    ...
    if Nm>0:
        ...
        plt.plot(spsi[i,0:Nm],806.2e5*sene[i,0:Nm]*q2m/f**2,'k.')
