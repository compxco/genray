...
c     theta_pol_wall(i=1,..,n_wall)
c     theta_pol_limiter(i=1,...,n_.imiter)
c     rho_wall(i=1,..,n_wall)
c     rho_limiter(i=1,..,n_wall)
      implicit none
      include 'param.i'
      include 'one.i'
      include 'three.i'
...
c-----output data
c     theta_pol_wall(n_wall),theta_pol_limiter(n_limiter)
...
c        and the poloidal angle thetapol_wall
      ...
      do i=1,n_limiter
         x_l=r_limiter(i)-xma 
         y_l=z_limiter(i)-yma
         call theta_xy(x_l,y_l,thetapol_limiter(i))
c         thetapol_limiter(i)=thetapol(z_limiter,r_limiter)
c--------calculate coordinates x_b,z_b at the LCFS surface psi=psilim
c        and the poloidal angle thetapol_wall

         call zr_psith(psilim,thetapol_limiter(i),z_b,r_b)

         l_zr=dsqrt(x_l**2+y_l**2)                   !distance between the magnetic axis 
                                                     !and the limite point
         l_z_b_r_b=dsqrt((r_b-xma)**2+(z_b-yma)**2)  !distance between the magnetic axis and
                                                     !LCFS at thetapol_wall(i)
         rho_limiter(i) = l_zr / l_z_b_r_b
