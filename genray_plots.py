...
level_mx= 4 #45 # Highest level of omega/omega_c to be plotted
...
istruct=1 # To skip, set istruct=0
...
mpl.rcParams['text.fontsize']=fnt+4
...
if i_ox==2:  # only for O-X transmission case
    ...
    if istart==1:    
        ...
        y_mn= np.amin(alphast[:])
        y_mx= np.amax(alphast[:])
        dy= 0.35*(y_mx-y_mn)
        plt.ylim((y_mn-dy,y_mx+dy))
        for i in range(0,Nrays,1):  # i goes from 0 to Nrays-1
            if remainder(i,6)==0: col='b'
