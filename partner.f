...
c     Both above cases also output genray calculated deposition and
c     current drive profiles to the netCDF file 'genray_profs_out.nc'.
...
c     suitable for the Plasma State, are output into the general
...
c --- some stuff for netCDF file ---
 ...
 1003 format (10i5)
      ...
      call ncopn2('genray_profs_in.nc',NCNOWRIT,ncid,istatus)
c      write(*,*)'after ncopn ncid=',ncid,'istatus',istatus

c     read in dimension IDs

      nj_id =ncdid2(ncid,'nj_dim',istatus)
c      write(*,*)'partner: after ncdid nj_id',nj_id,'istatus',istatus
      nspec_id =ncdid2(ncid,'nspecgr_dim',istatus)
c      write(*,*)'partner: after ncdid nspec_id=',nspec_id,'istatus',
c     .     istatus
      char256_id =ncdid2(ncid,'char256dim',istatus)
c      write(*,*)'partner: after ncdid char256_id=',char256_id,'istatus',
c     .     istatus

c --- inquire about dimension sizes:#species,grid size---
      call ncdinq2(ncid,nj_id,name,nj,istatus)
      call ncdinq2(ncid,nspec_id,name,nspec,istatus)
      call ncdinq2(ncid,char256_id,name,char256,istatus)
...
c-------------------------------------------------------------------------
 ...
 1001 format (5(1pe16.9))
      ...
      call ncvid2(vid,ncid,'eqdsk_name',istatus)
      call ncvgtc2(ncid,vid,1,char256,eqdsk_name,char256,istatus)
      write(*,*)'partner: eqdsk_name = ',trim(eqdsk_name)
c     trimming length of eqdsk_name to first blank
      ...
      call ncvid2(vid,ncid,'r',istatus)
      call ncvgt_doubl2(ncid,vid,1,nj,r_transport,istatus) !normalized rho
      write(*,*)'partner: r_transport'
      write(*, 1001)  (r_transport(j), j=1,nj)

      call ncvid2(vid,ncid,'zeff',istatus)   
      call ncvgt_doubl2(ncid,vid,1,nj,zeff,istatus)  !normalized rho
      write(*,*)'partner: zeff'
      write(*, 1001)  (zeff(j), j=1,nj)

      call ncvid2(vid,ncid,'charge',istatus)   
      call ncvgt_doubl2(ncid,vid,1,nspec,charge,istatus) !normalized rho
      call ncvid2(vid,ncid,'dmass',istatus)
      call ncvgt_doubl2(ncid,vid,1,nspec,dmass,istatus)  !normalized rho
      do i=1,nspec
         start(2)=i
         count(2)=1
         write(*,*)'partner: start=',start
         write(*,*)'partner: count=',count
         call ncvid2(vid,ncid,'en',istatus)   
         call ncvgt_doubl2(ncid,vid,start,count,en(1,i),istatus)  !normalized rho
         call ncvid2(vid,ncid,'temp',istatus)   
         call ncvgt_doubl2(ncid,vid,start,count,temp(1,i),istatus)  !normalized rho
      enddo
      start(2)=1
      count(2)=nspec
...
c-----current  density (along B field) profile
      ...
      if (nbulk.gt.1) then
         ...
         enddo
         if(myrank.eq.0) write(igenray,3010) (pgri_transport(j), j=1,nj)
...
cl    Define dimensions for netcdf file
      nprofs_id=ncddef2(ncid,'nprofs_dim',nj,istatus)
      nbulk_id=ncddef2(ncid,'nbulk_dim',nbulk,istatus)
      if (nbulk.gt.1) 
     .     nbulkm_id=ncddef2(ncid,'nbulkm_dim',nbulk-1,istatus)
...
c$$$Don't Need
c$$$      vid=ncvdef2(ncid,'nj',NCLONG,0,0,istatus)
c$$$      call ncaptc2(ncid,vid,'long_name',NCCHAR,24,
...
c$$$Don't Need
c$$$      vid=ncvdef2(ncid,'nbulk',NCLONG,0,0,istatus)
c$$$      call ncaptc2(ncid,vid,'long_name',NCCHAR,40,
c$$$     +           'Number of plasma species, incl electrons',istatus)
c$$$      call check_err(istatus)
c$$$


      vid=ncvdef2(ncid,'rgenray',NCDOUBLE,1,nprofs_id,istatus)
     ...
     +           'cms',istatus)
      ...
      vid=ncvdef2(ncid,'pgre',NCDOUBLE,1,nprofs_id,istatus)
     ...
     +           'Watts/cm**3',istatus)
      ...
      vid=ncvdef2(ncid,'totgrpe',NCDOUBLE,0,0,istatus)
     ...
     +           'Watts',istatus)
      ...
      if (nbulk.gt.1) then
         vid=ncvdef2(ncid,'pgri',NCDOUBLE,2,dimsm_id,istatus)
     ...
     +        'Watts/cm**3',istatus)
         ...
         vid=ncvdef2(ncid,'pgrit',NCDOUBLE,1,nprofs_id,istatus)
         call ncaptc2(ncid,vid,'long_name',NCCHAR,39,
     ...
     +        'Watts/cm**3',istatus)
         ...
         vid=ncvdef2(ncid,'totgrps',NCDOUBLE,1,nbulkm_id,istatus)
     ...
     +        'Watts',istatus)
         ...
         vid=ncvdef2(ncid,'totgrpi',NCDOUBLE,0,0,istatus)
     ...
     +        'Watts',istatus)
      ...
      vid=ncvdef2(ncid,'totgrp',NCDOUBLE,0,0,istatus)
     ...
     +     'Watts',istatus)
      ...
      vid=ncvdef2(ncid,'pgrc',NCDOUBLE,1,nprofs_id,istatus)
      call ncaptc2(ncid,vid,'long_name',NCCHAR,28,
     ...
     +           'Amps/cm**2',istatus)
      ...
      vid=ncvdef2(ncid,'totgrc',NCDOUBLE,0,0,istatus)
...
c     Write data into netcdf file
      ...
      call ncvid2(vid,ncid,'rgenray',istatus)
      call ncvpt_doubl2(ncid,vid,start(1),countm(1),r_transport,istatus)

      call ncvid2(vid,ncid,'pgre',istatus)
      call ncvpt_doubl2(ncid,vid,start(1),countm(1),pgre_transport,
     +                  istatus)

      call ncvid2(vid,ncid,'totgrpe',istatus)
      call ncvpt_doubl2(ncid,vid,1,1,totgrpe,istatus)

      if (nbulk.gt.1) then
         call ncvid2(vid,ncid,'pgri',istatus)
         do k=1,nbulk-1
            start(2)=k
            countm(2)=1
            call ncvpt_doubl2(ncid,vid,start,countm,pgrs_transport(1,k),
     .           istatus)
         enddo
         start(2)=1
         countm(2)=nbulk-1

         call ncvid2(vid,ncid,'pgrit',istatus)
         call ncvpt_doubl2(ncid,vid,start(1),countm(1),pgri_transport,
     +                     istatus)
         
         call ncvid2(vid,ncid,'totgrps',istatus)
         call ncvpt_doubl2(ncid,vid,start(2),countm(2),totgrps,istatus)
         
         call ncvid2(vid,ncid,'totgrpi',istatus)
         call ncvpt_doubl2(ncid,vid,1,1,totgrpi,istatus)
      endif

      call ncvid2(vid,ncid,'totgrp',istatus)
      call ncvpt_doubl2(ncid,vid,1,1,totgrp,istatus)

      call ncvid2(vid,ncid,'pgrc',istatus)
      call ncvpt_doubl2(ncid,vid,start(1),countm(1),pgrc_transport,
     +                  istatus)

      call ncvid2(vid,ncid,'totgrc',istatus)
      call ncvpt_doubl2(ncid,vid,1,1,totgrc,istatus)
