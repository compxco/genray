#!/bin/bash
# November 24, 2020 modified by AuP
...
#define XDRAW1 executable location 
#
JXDRAW="no"
...
#
#define JCEMACS="yes" to compare test files with emacs else "no" 
#
JCEMACS="no"

#
#define JCGS="yes" to compare test ps-files with evince else "no" 
#
JCEVINCE="no"
...
time $XGENRAY > log_test1   #1m30s on bob8  (1m48s, compx1 w bounds-check)
                            #23s on CompX2
if [ -f "con1" ]; then
rm  con1 #eps* gr3d* *.sap
fi
if [ -f "genray.doc" ]; then
rm genray.doc
fi
if [ -f "zrn.dat" ]; then
rm zrn.dat
...
if [ "$JCEVINCE" = "yes" ]; then
evince xdraw_genr_SS_Xmode_180_emission.ps&
evince xdraw_em_SS_Xmode_180_emission.ps&
